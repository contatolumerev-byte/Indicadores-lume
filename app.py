...
# =====================================================
# 1. ESTRUTURA DE MISSÕES POR REGIONAL
# =====================================================
...
MISSOES_PT = {
    ...
    "Regional Brasil Nordeste I": [
        "Teresina (Sede)", "Crateús", "Parnaíba", "Itapipoca", "Sobral",
        "Aracati", "Quixadá", "São Luís", "Imperatriz", "Juazeiro do Norte"
    ],

    "Regional Brasil Nordeste II": [
        "Salvador (Sede)", "Natal", "Mossoró", "Patos", "Recife", "Garanhuns",
        "Maceió", "Arapiraca", "João Pessoa", "Campina Grande", "Juazeiro - BA",
        "Vitória da Conquista/BA", "Aracaju", "Petrolina", "Propriá",
        "Senhor do Bonfim", "Eunápolis"
    ],

    "Regional Brasil Norte": [
        "Belém (Sede)", "Chaves", "Cruzeiro do Sul", "Macapá", "Manaus"
    ],

    "Regional Brasil Sudeste e Centro-Oeste": [
        "Brasília (Sede)", "Goiânia", "Cuiabá", "Campo Grande", "Palmas",
        "Rio de Janeiro", "Niterói", "Campos", "Juiz de Fora", "Vitória",
        "Belo Horizonte", "Patos de Minas", "Uberaba"
    ],

    "Regional Brasil São Paulo": [
        "São Paulo (Sede)", "Santo Amaro", "Aparecida", "Araraquara",
        "Guarulhos", "Santo André", "Piracicaba"
    ],

    "Regional Brasil Sul": [
        "Curitiba (Sede)", "Joinville", "Florianópolis", "Ponta Grossa", "São Leopoldo"
    ],

    "Regional América Hispânica": [
        "Assunção (Sede)", "Montevidéu", "Sucre", "Corrientes", "Lima",
        "Guayaquil", "Bogotá", "Santa Cruz de la Sierra", "Cidade do Panamá",
        "Santiago do Chile"
    ],

    "Regional Europa": [
        "Roma", "Ferrara", "Acqui Terme", "Lugano", "Granada", "Braga",
        "Setúbal", "Lisboa", "Toulon", "Aix-en-Provence", "Avignon",
        "Guiana Francesa", "Perpignan", "Nice", "Varsóvia", "Cracóvia",
        "Budapeste", "Paderborn", "Londres"
    ],

    "Regional Ásia": [
        "Haifa (Sede)", "Nazaré", "Manila (Filipinas)", "Hsinchu (Taiwan)"
    ],

    "Regional África": [
        "Maputo (Sede)", "Praia (Cabo Verde)", "Lubango", "Madagascar",
        "Argel", "Béjaia", "Sfax", "Tânger"
    ],

    "Regional América do Norte": [
        "Nova Iorque (Sede)", "Boston", "Toronto"
    ]
}

FORTALEZA_REGIONAIS = [
    "Parangaba", "Parquelândia", "Pacajus", "Aldeota",
    "Fátima", "Cidade dos Funcionários"
]

SETOR_DIFUSAO_OBRA = "Setor da Difusão da Obra"
...
def detect_indicator_format(indicator_name):
    ...
    if any(w in indicator_lower for w in ["taxa", "percentual", "índice", "porcentagem", "margem", "proporção"]):
        return "%"
    elif any(w in indicator_lower for w in ["número", "quantidade"]):
        return "#"
    elif any(w in indicator_lower for w in ["resultado", "valor", "financeiro"]):
        return "$"
    else:
        return "%"
...
CONFIANCA_PT_MAP = {
    ...
    "System": "Sistema", "Spreadsheet": "Planilha", "Guess/Estimate": "Chute/Estimativa",
    "Hoja de cálculo": "Planilha", "Estimación": "Chute/Estimativa",
    "Foglio di calcolo": "Planilha", "Stima": "Chute/Estimativa",
    "Système": "Sistema", "Feuille de calcul": "Planilha", "Estimation": "Chute/Estimativa",
    "Tabellenkalkulation": "Planilha", "Schätzung": "Chute/Estimativa"
}
...
for key in ["user", "perfil", "corrigindo_id", "mostrando_historico"]:
...
def aplicar_filtro_acesso(query, perfil):
    nivel = perfil.get("nivel_acesso")

    if nivel in ["admin", "diaconia_geral"]:
        return query
    if nivel == "assessoria_geral":
        return query.eq("assessoria_assistencia", perfil.get("assessoria_assistencia"))
    if nivel == "setor_difusao_obra":
        return (query
                .eq("assessoria_assistencia", "Assistência Apostólica")
                .eq("setor_servico", SETOR_DIFUSAO_OBRA)
                .eq("tipo_unidade", "difusao_obra"))
    if nivel == "regional":
        return (query
                .eq("regional", perfil.get("regional"))
                .neq("tipo_unidade", "difusao_obra"))
    if nivel == "missao_local":
        return (query
                .eq("tipo_unidade", "missao_local")
                .eq("missao", perfil.get("missao")))
    if nivel == "difusao_obra":
        return (query
                .eq("tipo_unidade", "difusao_obra")
                .eq("obra_difusao", perfil.get("obra_difusao")))
    if nivel == "fortaleza_regional":
        return (query
                .eq("tipo_unidade", "fortaleza_regional")
                .eq("regional_fortaleza", perfil.get("regional_fortaleza")))

    return query.eq("usuario_id", perfil.get("id"))


def buscar_historico(id_registro):
    """Busca histórico em duas queries separadas para evitar or_() frágil."""
    try:
        v1 = supabase.table("historico_indicadores").select("*").eq("id", id_registro).execute()
        v2 = supabase.table("historico_indicadores").select("*").eq("versao_anterior_id", id_registro).execute()
        dados = (v1.data or []) + (v2.data or [])
        dados.sort(key=lambda x: x.get("data_preenchimento", ""), reverse=True)
        return dados
    except Exception as e:
...
def tela_login():
    ...
    aba_login, aba_cadastro = st.tabs(["Entrar", "Criar acesso"])
    ...
    with aba_login:
        ...
        if st.button("Entrar", use_container_width=True):
            try:
                ...
                if not user:
                    st.error("Não foi possível autenticar este usuário.")
                    st.stop()

                perfil = carregar_perfil_usuario(user.id)
                if not perfil:
                    st.warning("Usuário autenticado, mas sem perfil ativo liberado.")
                    st.stop()
    ...
    with aba_cadastro:
        ...
        tipo_cadastro = st.selectbox(
            "Tipo de atuação",
            ["Missão Local", "Obra de Difusão", "Regional Fortaleza"]
        )

        regional_cad = missao_cad = obra_difusao_cad = None
        regional_fortaleza_cad = assessoria_cad = setor_servico_cad = None
        tipo_unidade_cad = nivel_acesso_cad = None

        if tipo_cadastro == "Missão Local":
            tipo_unidade_cad, nivel_acesso_cad = "missao_local", "missao_local"
            regional_cad = st.selectbox("Regional", list(MISSOES_PT.keys()))
            missao_cad = st.selectbox("Missão", MISSOES_PT[regional_cad])
            assessoria_cad = st.selectbox("Assessoria / Assistência", list(DB_STRUCTURE_PT.keys()))

        elif tipo_cadastro == "Obra de Difusão":
            tipo_unidade_cad, nivel_acesso_cad = "difusao_obra", "difusao_obra"
            regional_cad = st.selectbox("Regional de referência", list(MISSOES_PT.keys()))
            missao_cad = st.selectbox("Missão de referência", MISSOES_PT[regional_cad])
            obra_difusao_cad = st.text_input("Nome da Obra de Difusão")
            assessoria_cad = "Assistência Apostólica"
            setor_servico_cad = SETOR_DIFUSAO_OBRA
            st.info(
                "A Obra de Difusão ficará ligada à Assistência Apostólica, "
                "no Setor da Difusão da Obra. A Missão de Referência não recebe acesso automático."
            )

        elif tipo_cadastro == "Regional Fortaleza":
            tipo_unidade_cad, nivel_acesso_cad = "fortaleza_regional", "fortaleza_regional"
            regional_cad, missao_cad = "Diaconia - Governo Geral", "Fortaleza"
            regional_fortaleza_cad = st.selectbox("Regional de Fortaleza", FORTALEZA_REGIONAIS)
            assessoria_cad = st.selectbox("Assessoria / Assistência", list(DB_STRUCTURE_PT.keys()))

        if st.button("Criar acesso", use_container_width=True):
            try:
                ...
                if user:
                    supabase.table("perfis_usuarios").insert({
                        ...
                        "email": email_cad,
                        "nome": nome,
                        "nivel_acesso": nivel_acesso_cad,
                        "regional": regional_cad,
                        "missao": missao_cad,
                        "assessoria_assistencia": assessoria_cad,
                        "setor_servico": setor_servico_cad,
                        "tipo_unidade": tipo_unidade_cad,
                        "obra_difusao": obra_difusao_cad,
                        "regional_fortaleza": regional_fortaleza_cad,
                        "acesso_looker": False,
                        "ativo": True
                    }).execute()
                    st.success("Acesso criado com sucesso. Agora você já pode entrar.")
                else:
                    st.warning("Cadastro iniciado. Verifique seu e-mail, se a confirmação estiver ativa.")

            except Exception as e:
                st.error(f"Erro ao criar acesso: {e}")
...
with st.sidebar:
    st.markdown("---")
    st.markdown(f"**Usuário:** {perfil_usuario.get('email')}")
    st.markdown(f"**Acesso:** {perfil_usuario.get('nivel_acesso')}")
    if perfil_usuario.get("tipo_unidade"):
        st.markdown(f"**Unidade:** {perfil_usuario.get('tipo_unidade')}")
    if st.button("Sair"):
        logout()
...
regional_pt   = perfil_usuario.get("regional")
missao_pt     = perfil_usuario.get("missao")
setor_pt      = perfil_usuario.get("assessoria_assistencia")
setor_servico = perfil_usuario.get("setor_servico")
tipo_unidade  = perfil_usuario.get("tipo_unidade")
obra_difusao  = perfil_usuario.get("obra_difusao")
regional_fortaleza = perfil_usuario.get("regional_fortaleza")
...
with col_a:
    st.info(f"**Regional:** {regional_pt or 'Todas'}")

with col_b:
    if tipo_unidade == "difusao_obra":
        st.info(f"**Obra de Difusão:** {obra_difusao}")
    elif tipo_unidade == "fortaleza_regional":
        st.info(f"**Regional Fortaleza:** {regional_fortaleza}")
    else:
        st.info(f"**Missão:** {missao_pt or 'Todas'}")

with col_c:
    ano = st.selectbox(t["year_label"], [2024, 2025, 2026])

st.info(f"**Assessoria / Assistência:** {setor_pt or 'Todas'}")
if setor_servico:
    st.info(f"**Setor / Serviço:** {setor_servico}")
...
try:
    query = supabase.table("historico_indicadores").select("*").eq("versao_ativa", True)
    query = aplicar_filtro_acesso(query, perfil_usuario)
    registros_ativos = query.execute().data or []
except Exception as e:
    registros_ativos = []
...
st.markdown("## 📊 Meu progresso de preenchimento")

indicadores_setor = DB_STRUCTURE_PT.get(setor_pt, [])
total_indicadores = len(indicadores_setor)
total_esperado    = total_indicadores * 3
...
preenchidos = {
    ...
    if r.get("assessoria_assistencia") == setor_pt
}

feitos_total     = len(preenchidos)
percentual_geral = (feitos_total / total_esperado * 100) if total_esperado > 0 else 0

st.markdown(f"**Total:** {feitos_total}/{total_esperado} ({percentual_geral:.0f}%)")
...
for ano_ref, col_ref in [(2024, col_2024), (2025, col_2025), (2026, col_2026)]:
    with col_ref:
        feitos_ano    = sum(1 for (_, an) in preenchidos if an == ano_ref)
        pct_ano       = (feitos_ano / total_indicadores * 100) if total_indicadores > 0 else 0
        icon          = "✅" if pct_ano == 100 else "⚠️" if pct_ano > 0 else "❌"
        st.markdown(f"**{ano_ref}: {icon}**")
        st.markdown(f"{feitos_ano}/{total_indicadores} ({pct_ano:.0f}%)")
        st.progress(min(pct_ano / 100, 1.0))

st.markdown("---")


# =====================================================
# RESUMO DE DADOS REGISTRADOS
# =====================================================

st.markdown("## ✅ Dados registrados")

if registros_ativos:
    def formatar_valor(r):
        tipo, v = r.get("tipo_valor"), r.get("valor")
        if tipo == "%":  return f"{v}%"
        if tipo == "$":  return f"R$ {v:,.0f}".replace(",", ".")
        return str(v)
    ...
    dados_tabela = [
        {
            ...
            "Missão":            r.get("missao"),
            "Obra de Difusão":   r.get("obra_difusao"),
            "Reg. Fortaleza":    r.get("regional_fortaleza"),
            "Assessoria":        r.get("assessoria_assistencia"),
            "Setor/Serviço":     r.get("setor_servico"),
            "Indicador":         r.get("indicador"),
            "Ano":               r.get("ano"),
            "Valor":             formatar_valor(r),
            "Confiança":         r.get("confianca"),
            "Usuário":           r.get("usuario_email"),
        }
        for r in registros_ativos
    ]
    ...
    for r in registros_ativos:
        ...
        with col_ind:
            st.write(f"**{r.get('indicador')}** ({r.get('ano')})")
        with col_correct:
    ...
    st.info("Nenhum dado registrado ainda para o seu acesso.")
...
if st.session_state.corrigindo_id:
    ...
    try:
        resultado = supabase.table("historico_indicadores").select("*").eq(
            "id", st.session_state.corrigindo_id
        ).execute()

        if resultado.data:
            original = resultado.data[0]
            ...
            with col_form:
                novo_valor = st.number_input(t["val_label"], value=int(original["valor"]), step=1)
                motivo = st.selectbox(t["conf_label"], t["correction_reasons"])
                motivo_detalhado = (
                    st.text_input("Explicar motivo")
                    if motivo == t["correction_reasons"][-1]
                    else motivo
                )
            ...
            with col_btn1:
                if st.button(t["btn_confirm_correct"], use_container_width=True):
                    ...
                    supabase.table("historico_indicadores").insert({
                        ...
                        "usuario_email":         perfil_usuario.get("email"),
                        "tipo_unidade":          original.get("tipo_unidade"),
                        "obra_difusao":          original.get("obra_difusao"),
                        "regional_fortaleza":    original.get("regional_fortaleza"),
                    }).execute()
...
if st.session_state.mostrando_historico:
    ...
    versoes = buscar_historico(st.session_state.mostrando_historico)

    if versoes:
        for idx, v in enumerate(versoes, 1):
            status = "✅ ATIVO" if v["versao_ativa"] else "🔄 CORRIGIDO"
            motivo = f" ({v['motivo_correcao']})" if v.get("motivo_correcao") else ""
            st.write(f"**v{idx}** {status}{motivo}")
            st.caption(f"Valor: {v['valor']} | {v['data_preenchimento'][:10]}")
    else:
        st.info("Nenhuma versão anterior encontrada.")
...
if perfil_usuario.get("acesso_looker") and LOOKER_URL:
    st.markdown("## 📈 Painel Analítico")
    st.link_button("Abrir painel no Looker Studio", LOOKER_URL, use_container_width=True)
    st.markdown("---")
...
if not setor_pt:
    st.error("Seu perfil não possui assessoria/assistência definida.")
    st.stop()

if setor_pt not in DB_STRUCTURE_PT:
    st.error(f"A assessoria '{setor_pt}' ainda não possui indicadores cadastrados.")
    st.stop()

indicadores_display = INDICATORS_TRANSLATIONS.get(sel_lang, {}).get(setor_pt) or DB_STRUCTURE_PT[setor_pt]
...
if ja_preenchido:
    dados_existentes = preenchidos[(indicador_pt, ano)]
    st.warning(t["msg_already_filled"].format(ano=ano))
    st.info(f"Valor registrado: {dados_existentes['valor']} | Confiança: {dados_existentes['confianca']}")
else:
...
with col_valor:
    if formato == "%":
        ...
        valor = st.number_input(t["val_label"] + " (R$, valor inteiro)", min_value=0, step=1)

    # Aviso para valor zero em campos não-percentuais
    if formato in ("#", "$") and valor == 0:
        st.warning("⚠️ O valor está em 0. Confirme se é intencional antes de salvar.")
...
if st.button(t["btn_save"], use_container_width=True, disabled=ja_preenchido):
    supabase.table("historico_indicadores").insert({
        ...
        "obra_difusao":         obra_difusao,
        "regional_fortaleza":   regional_fortaleza,
        "missao_referencia":    missao_pt if tipo_unidade == "difusao_obra" else None,
    }).execute()
...
st.markdown(
    ...
    "Desenvolvido por Lume Rev Consultoria"
    "</div>",
